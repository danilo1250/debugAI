...
function canAnalyze(user) {
  ...
  if (!resetDate || resetDate.getMonth() !== now.getMonth() || resetDate.getFullYear() !== now.getFullYear()) {
    ...
    return { allowed: true, remaining: plan.analysisPerMonth, needsReset: true };
  }

  const remaining = plan.analysisPerMonth - user.analysis_count;
  return {
    allowed: remaining > 0,
...
function getPlanInfo(user) {
  ...
  return {
    plan: user.plan || "free",
    planName: plan.name,
    analysisLimit: plan.analysisPerMonth === -1 ? "ilimitado" : plan.analysisPerMonth,
